...
public class CustomStudentRepositoryImpl implements CustomStudentRepository {
	...
	public List<StudentDTO> getAllByClassId(Long id) {
		...
		sql.append("SELECT s.id, s.name, s.gender, s.is_aee as isAEE FROM student s ");
	...
	public StudentDTO getById(Long id) {
		...
		sql.append(" SELECT s.id, s.name, s.email, s.date_of_birth as dateBirth, s.phone, s.responsible_name as responsibleName, s.responsible_phone as responsiblePhone, " +
				" s.address, s.previous_school as previousSchool, s.observation, s.gender, s.registration_number as registrationNumber, s.is_aee as isAEE FROM student s" +
				" WHERE s.id = ? ");
