...
public class CustomStudentRepositoryImpl implements CustomStudentRepository {
	...
	public List<StudentDTO> getAllByClassId(Long id) {
		...
		sql.append("SELECT s.id, s.gender, s.name, s.isaee FROM student s ");
	...
	public List<StudentDTO> getAllByTeacherId(Long id) {
		StringBuilder sql = new StringBuilder();
		sql.append("SELECT s.id, s.name, s.gender, s.isaee FROM student s ");
	...
	public StudentDTO getById(Long id) {
		...
		sql.append(" SELECT s.id, s.name, s.email, s.date_of_birth as dateBirth, s.phone, s.responsible_name as responsibleName, s.responsible_phone as responsiblePhone, " +
				" s.address, s.previous_school as previousSchool, s.observation, s.gender, s.isaee, s.registration_number as registrationNumber FROM student s" +
				" WHERE s.id = ? ");
